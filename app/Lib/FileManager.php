...
class FileManager
{
    ...
    */
	protected function uploadImage(){
        ...
        // dd($this->file, $this->size, $this->filename);
    ...
    */
	public function makeDirectory($location = null){
		if (!$location) $location = public_path($this->path);
    ...
    */
	...
	{
		if (!$path) $path = public_path($this->path) . '/' . $this->old;

	    file_exists($path) && is_file($path) ? @unlink($path) : false;

	    if ($this->thumb) {
	    	if (!$path) $path = public_path($this->path) . '/thumb_' . $this->old;
	    	file_exists($path) && is_file($path) ? @unlink($path) : false;
	    }
	}
